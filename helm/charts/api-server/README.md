# pingmailer-api-server Helm chart

Helm chart for the Pingmailer **api-server** — the public HTTP entry point
for sending mail through the PingMailer stack. The Kubernetes counterpart of
...
| Key | Default | Notes |
|---|---|---|
| `image.repository` | `""` (**required**) | Your registry path, e.g. `ghcr.io/opengovmail/pingmailer-api-server` |
