...
readonly MAIL_DOMAIN=$(grep -m 1 '^\s*-\s*domain:' "${ROOT_DIR}/../conf/pingmailer.yaml" | sed 's/.*domain:\s*//' | xargs)
readonly LETSENCRYPT_PATH="${ROOT_DIR}/pingmailer-config/certbot/keys/etc/live/${MAIL_DOMAIN}"
readonly THUNDER_CERTS_PATH="${ROOT_DIR}/pingmailer-config/thunder/certs"
readonly THUNDER_DEPLOYMENT_FILE="${ROOT_DIR}/pingmailer-config/thunder/deployment.yaml"
readonly THUNDER_CONSOLE_CONFIG="${ROOT_DIR}/pingmailer-config/thunder/console-config.js"
readonly THUNDER_GATE_CONFIG="${ROOT_DIR}/pingmailer-config/thunder/gate-config.js"
