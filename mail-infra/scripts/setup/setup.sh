...
# ============================================
#  PingMailer Setup Script - This script is responsible for generating all the configs for our services.
# ============================================
...
readonly CONFIG_FILE="${CONF_DIR}/pingmailer.yaml"
# Read pingmailer-config from the configuration file
readonly PINGMAILER_CONFIG=$(grep -m 1 '^config-url:' "${CONFIG_FILE}" | sed 's/config-url: //' | xargs)
...
echo
echo -e " 🚀 ${GREEN}Welcome to the PingMailer Setup${NC}"
echo "---------------------------------------------"
